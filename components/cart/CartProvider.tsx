...
type CartContextValue={items:CartItem[];count:number;subtotalKobo:number;add:(item:Omit<CartItem,"quantity">)=>void;buyNow:(item:Omit<CartItem,"quantity">)=>void;remove:(slug:string)=>void;setQuantity:(slug:string,quantity:number)=>void;clear:()=>void};
...
export function CartProvider({children}:{children:React.ReactNode}){
 ...
 const value=useMemo(()=>({
  ...
  buyNow:(item:Omit<CartItem,"quantity">)=>setItems([{...item,quantity:1}]),
