...
export async function PremiumProductPage({ product }: { product: Product }) {
  ...
  const imageVersion = `6.3.6-${product.slug}`;
  ...
  return <main key={product.slug} className={`product-page ${product.theme}-theme`}>
    ...
    <nav className="product-anchor-nav" aria-label="Product page sections"><div className="container"><a href="#overview">Overview</a><a href="#specifications">Specifications</a><a href="#ingredients">Composition</a><a href="#directions">Directions</a><a href="#safety">Safety</a>{product.faqs.length ? <a href="#faq">FAQs</a> : null}<a href="#reviews">Reviews</a></div></nav>

    <section className="section" id="overview"><div className="container product-content-narrow"><SectionTitle eyebrow={product.slug === "aspivit" ? "ASPIVIT TABLET" : "Product efficacy"} title={product.slug === "aspivit" ? "Product efficacy" : `Why choose ${product.name}?`} intro={product.overview}/><div className="product-description-copy">{product.description.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div>{product.reasons.length ? <div className="why-grid">{product.reasons.map((reason) => <article className="product-info-card" key={reason.title}><BrandIcon symbol={reason.symbol}/><h3>{reason.title}</h3><p>{reason.text}</p></article>)}</div> : null}</div></section>
    ...
    <section className="section" id="ingredients"><div className="container"><SectionTitle eyebrow="Composition" title="What the formulation contains" intro="Ingredient strengths and concise descriptions are presented from the supplied product information."/><div className="ingredient-grid">{product.ingredients.map((ingredient) => <IngredientCard key={ingredient.name} icon={<BrandIcon symbol={ingredient.symbol}/>} name={ingredient.name} strength={ingredient.strength}>{ingredient.summary}</IngredientCard>)}</div></div></section>
    ...
    {product.detailImage ? <section className="section product-detail-flyer-section"><div className="container product-content-narrow"><SectionTitle eyebrow="Complete product guide" title={`${product.name} information flyer`}/><div className="product-detail-flyer"><Image src={`${product.detailImage}?v=${imageVersion}`} alt={`${product.name} detailed product information flyer`} width={1024} height={1536} unoptimized/></div></div></section> : null}
    ...
    <section className="section product-tint"><div className="container"><SectionTitle eyebrow="Related products" title="Explore more from Bridgecare"/><div className="related-grid">{related.map((entry) => <Link href={`/products/${entry.slug}`} className="related-product" key={entry.slug}><span className="related-image"><Image key={`related-${entry.slug}`} src={`${entry.image}?v=6.3.6-${entry.slug}`} alt={`${entry.name} product pack`} fill unoptimized sizes="180px"/></span><strong>{entry.name}</strong><small>{formatNaira(entry.priceKobo)}</small></Link>)}</div></div></section>
