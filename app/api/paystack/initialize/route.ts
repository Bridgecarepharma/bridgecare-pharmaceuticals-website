...
export async function POST(request:Request){
 ...
 try{
  if(!process.env.DATABASE_URL)return publicError("Checkout is temporarily unavailable. Please contact Bridgecare support.");
  ...
  const response=await fetch("https://api.paystack.co/transaction/initialize",{
   ...
   body:JSON.stringify({
    ...
    metadata:{order_id:order.id,order_number:order.orderNumber,shipping_kobo:shippingKobo,
     custom_fields:[
  ...
  return NextResponse.json({authorizationUrl:result.data.authorization_url,reference:paystackReference,orderNumber:order.orderNumber,totalKobo});
 }catch(error){
