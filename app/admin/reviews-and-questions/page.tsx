...
const reviewProducts = [
  { slug: "aspivit", name: "Aspivit® Tablet" },
  { slug: "asfenositol", name: "AsFenositol®" },
  { slug: "globivida", name: "Globivida® Capsules" },
  { slug: "herbal-bitter-tea", name: "Bridgecare Herbal Bitter Tea®" },
] as const;
...
async function updateReview(formData: FormData) {
  ...
  const requestedProductSlug = String(formData.get("productSlug") || "").trim();
  ...
  const status = (statusMap[quickAction as keyof typeof statusMap] ||
    (["PENDING", "APPROVED", "REJECTED", "ARCHIVED"].includes(requestedStatus)
      ? requestedStatus
      : "PENDING")) as "PENDING"|"APPROVED"|"REJECTED"|"ARCHIVED";

  const selectedProduct =
    reviewProducts.find((product) => product.slug === requestedProductSlug) || null;

  const adminReply =
    String(formData.get("adminReply") || "").trim().slice(0, 1500) || null;
  ...
  const [updated] = await prisma.$transaction([
    prisma.productReview.update({
      ...
      data: {
        ...
        isFeatured,
        ...(selectedProduct
          ? { productSlug: selectedProduct.slug, productName: selectedProduct.name }
          : {}),
      },
    }),
  ...
  if (existing?.productSlug) revalidatePath(`/products/${existing.productSlug}`);
