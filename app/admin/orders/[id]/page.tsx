...
export default async function OrderDetails({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ saved?: string }> }) {
  ...
  const isPayOnDelivery = order.paymentChannel === "PAY_ON_DELIVERY" || order.payment?.channel === "PAY_ON_DELIVERY";
  const isPendingPayment = !isPayOnDelivery && (order.status === "PENDING_PAYMENT" || !order.payment || ["PENDING", "FAILED", "ABANDONED"].includes(order.payment.status));
  ...
  const codWhatsappMessage = `Hello ${customerFirstName}, thank you for your Pay on Delivery order ${order.orderNumber} with Bridgecare Pharmaceuticals Limited. We are contacting you to confirm your Lagos delivery before dispatch. Your order total is ${naira(order.totalKobo)}. Please reply to confirm that you are available to receive and pay for the order.`;
  const paidWhatsappMessage = `Hello ${order.customerName}, this is Bridgecare Pharmaceuticals regarding order ${order.orderNumber}.`;
  const whatsappText = encodeURIComponent(isPayOnDelivery ? codWhatsappMessage : isPendingPayment ? pendingWhatsappMessage : paidWhatsappMessage);

  return <section className="section admin-shell"><div className="container"><AdminNav/>
    <div className="admin-heading admin-order-heading"><div><span className="eyebrow">Order management</span><h1>{order.orderNumber}</h1><p>Placed {adminDate(order.createdAt)} · {order.items.reduce((sum, item) => sum + item.quantity, 0)} pack(s)</p></div><div className="admin-order-actions"><Link className="button secondary" href={`/admin/orders/${order.id}/invoice`}>Invoice</Link><Link className="button secondary" href={`/admin/orders/${order.id}/packing-slip`}>Packing slip</Link><Link className="button secondary" href={`/admin/orders/${order.id}/waybill`}>Print waybill</Link><a className="button secondary" href={`https://wa.me/${whatsappPhone}?text=${whatsappText}`} target="_blank" rel="noreferrer">WhatsApp customer</a></div></div>
    {saved ? <div className="admin-success">Order updated successfully.</div> : null}
    <div className="admin-order-state">{isPayOnDelivery ? <span className="admin-status status-processing">PAY ON DELIVERY</span> : null}<span className={`admin-status status-${order.status.toLowerCase().replaceAll("_", "-")}`}>{order.status.replaceAll("_", " ")}</span><span className={`admin-status status-${(order.payment?.status || "PENDING").toLowerCase()}`}>Payment {order.payment?.status || "PENDING"}</span>{order.trackingNumber ? <span className="tracking-chip">Tracking: {order.trackingNumber}</span> : null}</div>
