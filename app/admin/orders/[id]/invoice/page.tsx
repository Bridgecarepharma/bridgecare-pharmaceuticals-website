...
export default async function InvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="admin-document-shell">
    ...
    <article className="admin-document">
      ...
      <div className="admin-document-totals"><p><span>Subtotal</span><strong>{naira(order.subtotalKobo)}</strong></p><p><span>Delivery</span><strong>{naira(order.shippingKobo)}</strong></p>{order.discountKobo > 0 && <p><span>Coupon {order.couponCode || "discount"}</span><strong>-{naira(order.discountKobo)}</strong></p>}<p className="grand"><span>Total</span><strong>{naira(order.totalKobo)}</strong></p></div>
